...
struct {
    __uint(type, BPF_MAP_TYPE_LRU_HASH);
    __uint(key_size, sizeof(struct modbus_key));
...
static inline void insert_modbus(struct modbus_key key){
    bool ms = true;
    bpf_map_update_elem(&modbus_state_map, &key, &ms,0);
}

static inline bool check_modbus_state(struct modbus_key key){
    bool *ms;
    ms = bpf_map_lookup_elem(&modbus_state_map, &key);
    if(ms){
	    return true;
    }else{
        return false;
...
static inline void del_modbus(struct modbus_key key){
    bpf_map_delete_elem(&modbus_state_map, &key);
}
...
int bpf_sk_splice(struct __sk_buff *skb){
    ...
    struct tuple_key udp_egress_state_key ={0};
    ...
    if(ipv4)
    {
        ...
        */
        ...
        else
        {
            ...
            unsigned long long tstamp = bpf_ktime_get_ns();
            event.proto = IPPROTO_UDP;
            udp_egress_state_key.__in46_u_src.ip = tuple->ipv4.saddr;
            udp_egress_state_key.__in46_u_dst.ip = tuple->ipv4.daddr;
            udp_egress_state_key.sport = tuple->ipv4.sport;
            udp_egress_state_key.dport = tuple->ipv4.dport;
                ...
                if(local_diag->masquerade && local_ip4 && local_ip4->count)
                {
                   ...
                   if(revv)
                   {
                       ...
                       if(mvptr)
                       {
                               ...
                               if(iph->saddr != local_ip4->ipaddr[0]){
                                       not_local = true;
                                       iph->saddr = local_ip4->ipaddr[0];
                                       /*Calculate l3 Checksum*/
                                       bpf_l3_csum_replace(skb, sizeof(struct ethhdr) + offsetof(struct iphdr, check), 0, l3_sum, 0);
                               ...
                               if(udph->check != 0)
                               {
                                       ...
                                       if(not_local){
                                               ...
                                               udph->source = rand_source_port;
                                               bpf_l4_csum_replace(skb, sizeof(struct ethhdr) + sizeof(struct iphdr) + offsetof(struct udphdr, check),  mv.o_sport, rand_source_port, flags);
        ...
        if(tcp){
            ...
            if(local_diag->ot_filtering && (tuple->ipv4.dport == bpf_ntohs(502)) && (tuple->ipv4.sport >= bpf_ntohs(1024)) && payload_len > 0){
                ...
                if(*modbus_proto == 0){               
                    ...
                    if(bpf_ntohs(*modbus_len) > 2){
                        ...
                        mb_state.type = 4;
                        mb_state.ifindex = event.ifindex;
                        mb_state.__in46_u_dst.ip = tuple->ipv4.saddr;
                        mb_state.__in46_u_src.ip = tuple->ipv4.daddr;
                        mb_state.dport = tuple->ipv4.sport;     
                        mb_state.ti = *modbus_ti;
            ...
            if(local_diag->masquerade && local_ip4 && local_ip4->count){
                ...
                if(not_local){
                    ...
                    tcph->source = rand_source_port;
                    bpf_l4_csum_replace(skb, sizeof(struct ethhdr) + sizeof(struct iphdr) + offsetof(struct tcphdr, check), mv.o_sport, rand_source_port, flags);
                }
                iph = (struct iphdr *)(skb->data + sizeof(*eth));
                if ((unsigned long)(iph + 1) > (unsigned long)skb->data_end){
                    return TC_ACT_SHOT;
            ...
            * with with ingress tc program */
            ...
            if(!local_diag->masquerade && !local_diag->outbound_filter){
                reverse_tuple.ipv4.daddr = tuple->ipv4.saddr;
                reverse_tuple.ipv4.dport = tuple->ipv4.sport;
                reverse_tuple.ipv4.saddr = tuple->ipv4.daddr;
                reverse_tuple.ipv4.sport = tuple->ipv4.dport;
